"""Model-backed checks that run only where the pinned snapshot is staged (local pre-flight): masked-token
metrics of the frozen model against the unigram floor, a one-epoch continued-MLM adaptation of the last
encoder layer on a dozen abstracts, and the artifact round trip. Skipped when the weights are absent."""
...
def test_frozen_masked_metrics_beat_the_unigram_floor(pipe):
    metrics = pipe.evaluate(RECORDS[8:])
    floor = pipe.unigram_baseline(RECORDS[:8], RECORDS[8:])
    assert (
        metrics["n_records"] == 4 and metrics["adapted"] is False and metrics["n_masked"] == floor["n_masked"]
    )
    assert (
        1.0 < metrics["perplexity"] < floor["perplexity"]
        and metrics["top5_accuracy"] >= floor["top5_accuracy"]
    )


def test_one_epoch_adaptation_and_artifact_round_trip(pipe, tmp_path):
    before = pipe.evaluate(RECORDS[:8])["perplexity"]
    result = pipe.adapt(RECORDS[:8], RECORDS[8:11], epochs=1, trainable_layers=1, batch_size=4)
    assert result["n_trainable"] == 7_087_872 and result["history"][0]["note"] == "frozen model"
    assert all(name.startswith("bert.encoder.layer.11.") for name in result["trainable_names"])
    assert result["n_total"] == 109_514_298 and "perplexity" in result["history"][1]["val"]
    history = result["history"]
    assert history[1]["val"]["perplexity"] != history[0]["val"]["perplexity"]  # the encoder moved
    assert (pipe.evaluate(RECORDS[:8])["perplexity"] == pytest.approx(before)) == (result["best_epoch"] == 0)
    ...
    reloaded = BERTMaskedLMPipeline.from_artifact(artifact, device="cpu")
    assert reloaded.evaluate(RECORDS[:4])["perplexity"] == pytest.approx(
        pipe.evaluate(RECORDS[:4])["perplexity"]
    )
    text = "We study graph neural networks for molecule [MASK] prediction."
    assert reloaded.fill_mask(text)["candidates"] == pipe.fill_mask(text)["candidates"]
    assert reloaded.embed([ABSTRACTS[0]])["embeddings"] == pipe.embed([ABSTRACTS[0]])["embeddings"]
...
def test_no_validation_keeps_the_final_epoch_and_reloads_it(pipe, tmp_path):
    ...
    result = pipe.adapt(RECORDS[:8], None, epochs=3, trainable_layers=1, batch_size=4)
...
def test_load_artifact_refuses_a_tensor_set_that_differs_from_the_recorded_configuration(pipe, tmp_path):
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        BERTMaskedLMPipeline.from_artifact(fewer, device="cpu")
    ...
    with pytest.raises(ValueError, match="tensor names differ"):
        BERTMaskedLMPipeline.from_artifact(extra, device="cpu")
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        BERTMaskedLMPipeline.from_artifact(other_layers, device="cpu")
...
def test_adapt_is_transactional_when_the_progress_callback_raises(pipe):
    ...
    with pytest.raises(RuntimeError, match="boom"):
        pipe.adapt(RECORDS[:8], None, epochs=2, trainable_layers=1, batch_size=4, progress=boom)
