"""Static release-asset validation for the BERT-Base uncased masked-LM / embedding DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    'outputs/bert_masked_lm_result.json',
    'outputs/bert_masked_lm_embeddings.csv',
)
# Profile-specific code the notebook must exercise through the repository public API.
CODE_MARKERS = (
    'from bert_masked_lm_pipeline import DEFAULT_TOP_K, HIDDEN_SIZE, MASK_TOKEN, MAX_BATCH, '
    'MAX_TEXT_CHARS, MAX_TEXT_TOKENS, MAX_TOP_K, POOLINGS, VOCAB_SIZE',
    'from bert_masked_lm_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, BERTMaskedLMPipeline, '
    ...
    "ceilings = {'MAX_TEXT_CHARS': MAX_TEXT_CHARS, 'MAX_TEXT_TOKENS': MAX_TEXT_TOKENS, "
    "'MAX_BATCH': MAX_BATCH, 'MAX_TOP_K': MAX_TOP_K, 'VOCAB_SIZE': VOCAB_SIZE, "
    "'HIDDEN_SIZE': HIDDEN_SIZE, 'MASK_TOKEN': MASK_TOKEN, 'POOLINGS': POOLINGS, "
    ...
    "'decision_rule': 'argmax (first candidate); score is a vocabulary softmax, "
    "not a calibrated probability; no threshold shipped'",
    "'model_revision'",
...
MARKDOWN_MARKERS = (
    '**Capability:** masked-language modelling (fill-mask: ranked candidates for one `[MASK]`) '
    'and sentence embeddings',
    '**No adaptation occurs:**',
    '**The fill-mask `score` is not a calibrated probability**',
    ...
    '`MAX_TEXT_TOKENS` (512, the checkpoint\'s position limit) is applied after tokenisation '
    'and **rejects** longer texts rather than truncating them',
    'fine-tuning or classification heads, next-sentence prediction, multi-mask filling',
    '**Input/output contract.** `fill_mask(text, top_k=...)`',
    '**Input/output contract.** `embed(texts, pooling=...)`',
)
