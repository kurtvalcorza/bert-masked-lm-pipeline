...
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/bert_masked_lm_train.csv",
    "outputs/bert_masked_lm_input_manifest.json",
    "outputs/bert_masked_lm_evaluation_report.json",
    "outputs/bert_masked_lm_cloze.csv",
    "outputs/bert_masked_lm_adapter",
...
CODE_MARKERS = (
    ...
    "'one_unit_vector_per_text': vectors.shape == (2, HIDDEN_SIZE)",
...
MARKDOWN_MARKERS = (
    ...
    "bounded continued masked-language-model training of the last encoder layers on a text corpus",
    "**The fill-mask `score` is not a calibrated probability**",
    "**embeddings are representations, not predictions**",
    "**continued pre-training measured by the model's own objective**",
    "**seeded 15 %**",
    "**add-one unigram model**",
