...
- the profile-specific public-API calls (`stage_missing_files`, `verify_snapshot`,
  `BERTMaskedLMPipeline.from_pretrained`, `fill_mask(cloze, top_k=TOP_K)`, `embed(sentences,
  pooling=POOLING)`), the ceiling constants imported from the package, the single-`[MASK]` check,
  the descending-score and vector-shape sanity checks, the identifier-keyed CSV export, the
  exported decision rule, the learner-facing statements (no adaptation, score not a calibrated
  probability, argmax rule, embeddings are representations, one vector per text, empty strings
  rejected, no intrinsic metric, no metric reported, reject-not-truncate, per-capability contracts)
  and the gated-off BYOD default listed in the validator; forbidden patterns (credential-in-URL,
  direct `transformers` or `huggingface_hub` calls that bypass the pipeline, `BertForMaskedLM`,
  `AutoTokenizer`, `AutoModel`, `trust_remote_code=True`, `pickle.load`, `torch.load(`, `extractall(`);
...
5. verify every default-path stage completes:
   ...
   - ceilings `MAX_TEXT_CHARS = 4000`, `MAX_TEXT_TOKENS = 512`, `MAX_BATCH = 64`, `MAX_TOP_K = 100`,
     ...
     printed and the inputs accepted before model execution;
   - `stage_missing_files(..., allow_download=True)` reporting `['model.safetensors']` fetched from
     ...
     8-entry manifest, and `BERTMaskedLMPipeline.from_pretrained` loading from the verified
   ...
   - `fill_mask` returning 5 candidates with descending scores in (0, 1], all five sanity checks
     true, the expected token `paris` present in the top-5 (an observation, not a metric), and the
     "no metric is reported" line printed;
   ...
   - `outputs/bert_masked_lm_embeddings.csv` written with one identified row per sentence and
     ...
     contract, identified texts, cosine table, repository SHA, model identifier, immutable model
     revision, snapshot summary, runtime versions, device and dtype;
