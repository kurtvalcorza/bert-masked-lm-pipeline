...
|---|---|---|---|---|
| `bert_masked_lm_colab.ipynb` (`E2E`) | `1c8c93b` / `1d474b2f` | 2026-09-19 | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-bert-masked-lm` v4; image `torch 2.10.0+cu128` / `transformers 5.0.0` before the pinned install, `torch 2.14.0+cu130` / `transformers 4.57.6` after, Python 3.12.13, `cuda:0`) | **PASSED** — 11/11 code cells ok (1 restart after install cell); 21 files, 447 MB staged from the Hub into a clean cache; comparison {perplexity: {unigram_floor: 1175, frozen: 15.09, adapted: 12.66}, bits_per_token: {unigram_floor: 10.2, frozen: 3.916, adapted: 3.662}, top1_accuracy: {unigram_floor: 0.0422, frozen: 0.5334, adapted: 0.5448}, top5_accuracy: {unigram_floor: 0.1488, frozen: 0.7013, adapted: 0.7211}, delta_vs_frozen: {perplexity: -2.437, bits_per_token: -0.254, top1_accuracy: 0.0114, top5_accuracy: 0.0198}}; reload parity {perplexity_in_memory: 14.74, perplexity_reloaded: 14.74, identical_candidates: 3, of: 3, embeddings_identical: True}; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-bert-masked-lm/v4/evidence/` in the workspace |
...
|---|---|---|---|---|---|
| 2026-09-19 | `1c8c93b` / `1d474b2f` | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-bert-masked-lm` v4; image `torch 2.10.0+cu128` / `transformers 5.0.0` before the pinned install, `torch 2.14.0+cu130` / `transformers 4.57.6` after, Python 3.12.13, `cuda:0`) | Default sample path, `Run all` from a fresh interpreter with an empty Hugging Face cache and no repository checkout (blob SHA-1 verified against GitHub before execution) | 272.5 s | **PASSED** — 11/11 code cells ok (1 restart after install cell); 21 files, 447 MB staged from the Hub into a clean cache; comparison {perplexity: {unigram_floor: 1175, frozen: 15.09, adapted: 12.66}, bits_per_token: {unigram_floor: 10.2, frozen: 3.916, adapted: 3.662}, top1_accuracy: {unigram_floor: 0.0422, frozen: 0.5334, adapted: 0.5448}, top5_accuracy: {unigram_floor: 0.1488, frozen: 0.7013, adapted: 0.7211}, delta_vs_frozen: {perplexity: -2.437, bits_per_token: -0.254, top1_accuracy: 0.0114, top5_accuracy: 0.0198}}; reload parity {perplexity_in_memory: 14.74, perplexity_reloaded: 14.74, identical_candidates: 3, of: 3, embeddings_identical: True}; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-bert-masked-lm/v4/evidence/` in the workspace |
